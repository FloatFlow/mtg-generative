...
class MSGGAN():
    ...
    def build_generator(self, ch=128):
        ...
        label_embed = EqualizedDense(units=self.n_classes, kernel_initializer=self.kernel_init)(label_in)
        x = Concatenate(axis=-1)([latent_in, label_embed])
        x = LatentPixelNormalization()(x)
        x = EqualizedDense(units=4*4*ch, gain=1, kernel_initializer=self.kernel_init)(x)
        x = Reshape((4, 4, -1))(x)
        ...
        out_4x = EqualizedConv2D(filters=3,
                                 kernel_size=1,
                                 padding='same',
                                 kernel_initializer=self.kernel_init)(x)
    ...
    def build_discriminator(self, ch=16, kernel_init='he_normal'):
        ...
        img_features = EqualizedConv2D(filters=ch,
                        kernel_size=1,
                        padding='same',
        ...
        x = Flatten()(x)
        #x = EqualizedDense(units=ch, kernel_initializer=self.kernel_init, gain=np.sqrt(2))(x)
        #x = LeakyReLU(0.2)(x)

        # architecture of tail stem
        out = EqualizedDense(units=1, kernel_initializer=self.kernel_init, gain=1)(x)
        y = EqualizedDense(units=1, kernel_initializer=self.kernel_init, gain=1)(class_in)
